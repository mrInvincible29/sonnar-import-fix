...
class ScoreAnalyzer:
    ...
    def analyze_custom_formats(self, custom_formats_list: List[Dict], series_id: int) -> FormatAnalysis:
        """
    ...
    def find_grab_info(self, history: List[Dict], download_id: Optional[str] = None, 
                      series_id: Optional[int] = None) -> Tuple[Optional[int], List[str]]:
        """
    ...
    def get_current_file_details(self, episode_id: int, series_id: Optional[int] = None) -> Tuple[Optional[int], List[str]]:
        ...
        try:
            ...
            logger.error(f"Failed to get current file details for episode {episode_id}: {e}")
            return None, []
    ...
    def analyze_queue_item(self, queue_item: Dict) -> Decision:
        ...
        # Get episode history
        history = self.sonarr_client.get_history_for_episode(episode_id) if episode_id else []
        ...
        self._log_analysis_details(queue_item, grab_score, current_score, 
                                 grab_formats, current_formats, indexer)
        ...
        return Decision(
            ...
            grab_score=grab_score,
            current_score=current_score,
            score_difference=grab_score - current_score if grab_score is not None and current_score is not None else None,
    ...
    def _find_indexer_from_history(self, history: List[Dict], download_id: Optional[str]) -> str:
    ...
    def _log_analysis_details(self, queue_item: Dict, grab_score: Optional[int], 
                            current_score: Optional[int], grab_formats: List[str], 
                            current_formats: List[str], indexer: str):
    ...
    def _make_decision(self, grab_score: Optional[int], current_score: Optional[int], 
                      is_private_tracker: bool, grab_formats: List[str], 
                      current_formats: List[str], missing_formats: List[str], 
                      extra_formats: List[str]) -> Dict[str, str]:
        """
        ...
        if score_diff >= self.force_import_threshold:
            ...
            if missing_formats:
                reasoning += f". Missing formats: {', '.join(list(missing_formats)[:3])}"
    ...
    def detect_repeated_grabs(self, episode_id: int) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Error detecting repeated grabs for episode {episode_id}: {e}")
            return []
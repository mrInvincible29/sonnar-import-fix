...
class SonarrImportMonitor:
    ...
    def start_webhook_server(self) -> bool:
        ...
        try:
            ...
            logger.info(f"   Configure Sonarr webhook URL: http://{host}:{port}/webhook/sonarr")
        ...
        for item in queue:
            ...
            if item.get('trackedDownloadState') == 'importPending':
                ...
                for msg in item.get('statusMessages', []):
                    ...
                    if any(keyword in message_text for keyword in ['already', 'exists', 'duplicate']):
                        stuck_items.append(item)
                        break
    ...
    def _execute_removal(self, queue_item: Dict) -> bool:
        ...
        success = self.sonarr_client.remove_from_queue(queue_id, remove_from_client=True, blocklist=False)
    ...
    def check_repeated_grabs(self) -> int:
        ...
        try:
            # Get recent history
            response = self.sonarr_client._make_request('GET', '/history', params={
                'pageSize': 200,
                'sortKey': 'date',
            ...
            for ep_id, grabs in episodes_with_grabs.items():
                if len(grabs) > 1:
                    ...
                    if unimported:
                        problem_episodes += 1
                        logger.warning(f"  Episode {ep_id}: {len(grabs)} grabs, {len(unimported)} unimported")
            ...
            if problem_episodes == 0:
                ...
                logger.warning(f"Found {problem_episodes} episodes with repeated grab issues")
    ...
    def test_specific_episode(self, series_title: str, season: int, episode: int):
        ...
        try:
            ...
            current_score, current_formats = self.analyzer.get_current_file_details(episode_id, series['id'])
            ...
            if queue_item:
                ...
                history = self.sonarr_client.get_history_for_episode(episode_id, limit=10)
                if history:
    ...
    def _show_history_analysis(self, history: List[Dict], episode_id: int, series_id: int):
        ...
        if import_events:
            ...
            for idx, imp in enumerate(import_events[:3], 1):
                ...
                status = "✓ Imported" if imp['eventType'] == 'downloadFolderImported' else "✗ Ignored"
                date = imp.get('date', '')
    ...
    def run_continuous(self, enable_webhook: bool = False):
        ...
        detect_repeated_grabs = self.config.get('monitoring.detect_repeated_grabs', True)
        ...
        logger.info(f"   Force Import Threshold: {self.config.get('decisions.force_import_threshold')}")
        ...
        try:
            while self.running:
                cycle_count += 1
                logger.info(f"\n{'='*60}")
                logger.info(f"🔄 Check cycle {cycle_count} at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

...
class SonarrClient:
    ...
    @retry(max_attempts=3, delay=1.0, exceptions=(SonarrAPIError, requests.RequestException))
    @log_execution_time()
    ...
    def get_episode_info(self, series_id: int, season: int, episode: int) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Failed to get episode S{season:02d}E{episode:02d} for series {series_id}: {e}")
            return None
    ...
    @retry(max_attempts=2, delay=2.0, exceptions=SonarrAPIError)
    def force_import(self, download_id: str, episode_id: int, quality: Dict = None) -> bool:
        """
        ...
        try:
            ...
            if not import_items:
                logger.warning(f"No manual import candidates found for download {download_id}")
                return False
    ...
    @retry(max_attempts=2, delay=1.0, exceptions=SonarrAPIError)
    def remove_from_queue(self, queue_id: int, remove_from_client: bool = True, blocklist: bool = False) -> bool:
        """

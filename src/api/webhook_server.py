...
class WebhookServer:
    ...
    def verify_webhook_signature(self, f: Callable) -> Callable:
        ...
        def decorated_function(*args, **kwargs):
            ...
            # Rate limiting check
            if not self.rate_limiter.is_allowed(client_ip, self.max_requests_per_minute, 60):
    ...
    def _authenticate_request(self) -> bool:
        ...
        if not self.webhook_secret:
            logger.warning("No webhook secret configured - allowing unauthenticated access")
            return True
        ...
        logger.warning(f"No authentication credentials provided from {request.remote_addr}")
        return False
    ...
    def setup_routes(self):
        ...
        def metrics():
            ...
            return jsonify({
                ...
                'requests_authenticated': self.metrics['requests_authenticated'],
        ...
        def webhook_info():
            ...
            return jsonify({
                ...
                'supported_events': ['Test', 'Grab', 'Download', 'ImportFailed', 'HealthIssue'],
    ...
    def _log_webhook_event(self, event_type: str, data: Dict[str, Any]):
        ...
        if episodes:
            ep = episodes[0]
            episode_str = f"S{ep.get('seasonNumber', 0):02d}E{ep.get('episodeNumber', 0):02d}"
        else:
            episode_str = "N/A"
        
        logger.info(f"📨 {event_type} webhook: {series_title} {episode_str} from {request.remote_addr}")
            ...
            for episode in episodes:
                ...
                if ep_id in self.grab_cache:
                    ...
                    if score_diff > threshold:
                        ...
                        logger.warning(f"   This shouldn't happen - investigate further")
    ...
    def _schedule_delayed_check(self, episode_id: int, download_id: str, delay: int):
        ...
        timer = threading.Timer(
            delay,
            self._check_if_imported,
            args=[episode_id, download_id]
        )
        timer.daemon = True
        timer.start()
    ...
    def _check_if_imported(self, episode_id: int, download_id: str):
        ...
        try:
            if episode_id not in self.grab_cache:
                logger.debug(f"Episode {episode_id} already cleared from cache (likely imported)")
                return
            ...
            if queue_item:
                ...
                logger.warning(f"⏰ Download still in queue after {self.config.get('webhook.import_check_delay', 600)}s delay")
                logger.info(f"   Status: {status}, State: {state}")
                
                if status == 'completed' or state == 'importPending':
                    logger.info("   Download completed but not importing - triggering analysis")
                ...
                if self._was_imported_silently(episode_id, download_id):
                    ...
                    logger.warning(f"   Episode never imported and not in queue - possible issue")
        
        except Exception as e:
            logger.error(f"Error checking delayed import: {e}")
    
    def _find_queue_item(self, queue: list, episode_id: int, download_id: str) -> Optional[Dict]:
    ...
    def _was_imported_silently(self, episode_id: int, download_id: str) -> bool:
        ...
        try:
            history = self.monitor.sonarr_client.get_history_for_episode(episode_id, limit=10)
    ...
    def start(self, host: str = '0.0.0.0', port: int = 8090):
        ...
        if self.webhook_secret:
            ...
            logger.info("   Configure Sonarr with X-Webhook-Secret header or HMAC signatures")
        else:

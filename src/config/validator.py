...
class ConfigValidator:
    ...
    def validate_url(url: str, schemes: List[str] = None) -> bool:
        ...
        try:
            ...
            if parsed.scheme not in schemes:
                raise ValidationError(f"URL scheme must be one of {schemes}, got: {parsed.scheme}")
    ...
    def validate_api_key(api_key: str) -> bool:
        ...
        placeholders = [
            'your-api-key', 'your-api-key-here', 'changeme', 
            'replace-me', 'example', 'test', 'placeholder'
        ]
    ...
    @staticmethod
    def validate_threshold(threshold: Any, min_val: int = 0, max_val: int = 1000) -> bool:
        """
        ...
        try:
            ...
            if not (min_val <= threshold_int <= max_val):
                raise ValidationError(f"Threshold must be between {min_val}-{max_val}, got: {threshold_int}")
            return True
        except (ValueError, TypeError):
            raise ValidationError(f"Threshold must be a valid integer, got: {threshold}")
    ...
    def validate_interval(interval: Any, min_val: int = 10) -> bool:
        ...
        try:
            ...
            if interval_int < min_val:
                raise ValidationError(f"Interval must be at least {min_val} seconds, got: {interval_int}")
            return True
        except (ValueError, TypeError):
    ...
    def validate_log_level(level: str) -> bool:
        ...
        if level.upper() not in valid_levels:
            raise ValidationError(f"Log level must be one of {valid_levels}, got: {level}")
    ...
    def validate_log_format(format_type: str) -> bool:
        ...
        if format_type.lower() not in valid_formats:
            raise ValidationError(f"Log format must be one of {valid_formats}, got: {format_type}")

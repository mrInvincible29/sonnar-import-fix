...
class TestSonarrImportMonitor:
    ...
    def monitor(self, mock_config):
        ...
        with patch('src.core.monitor.SonarrClient'), \
             patch('src.core.monitor.ScoreAnalyzer'), \
...
class TestConfigurationTesting:
    ...
    def test_test_configuration_success(self, monitor):
        ...
        monitor.sonarr_client.fetch_quality_profiles.return_value = {'1': {'name': 'HD'}}
...
class TestWebhookServerManagement:
    ...
    @patch('time.sleep')
    def test_start_webhook_server_success(self, mock_sleep, mock_thread, mock_webhook_class, monitor):
...
class TestStuckImportProcessing:
    ...
    def test_process_stuck_imports_with_actions(self, monitor_with_mocks):
        ...
        with patch.object(monitor_with_mocks, '_identify_stuck_items', return_value=stuck_queue):
            # Mock process_queue_item returns
            with patch.object(monitor_with_mocks, 'process_queue_item', side_effect=['forced_import', 'removed']):
                results = monitor_with_mocks.process_stuck_imports()
...
class TestQueueItemProcessing:
    ...
    def test_process_queue_item_force_import(self, monitor_with_mocks, mock_sonarr_queue_item):
        ...
        with patch.object(monitor_with_mocks, 'analyzer') as mock_analyzer:
            with patch.object(monitor_with_mocks, '_execute_force_import', return_value=True) as mock_execute:
    ...
    def test_process_queue_item_remove(self, monitor_with_mocks, mock_sonarr_queue_item):
        ...
        with patch.object(monitor_with_mocks, 'analyzer') as mock_analyzer:
            with patch.object(monitor_with_mocks, '_execute_removal', return_value=True) as mock_execute:
    ...
    def test_process_queue_item_error(self, monitor_with_mocks, mock_sonarr_queue_item):
        ...
        with patch.object(monitor_with_mocks, 'analyzer') as mock_analyzer:
            with patch.object(monitor_with_mocks, 'stats', {'errors_encountered': 0}):
                mock_analyzer.analyze_queue_item.side_effect = Exception("Analysis error")
...
class TestRepeatedGrabDetection:
    ...
    def test_check_episode_queue_found(self, monitor):
        ...
        monitor.process_queue_item.assert_called_once_with({'id': 1, 'episode': {'id': 100}})
...
class TestSpecificEpisodeTesting:
    ...
    def test_test_specific_episode_found(self, monitor):
        ...
        monitor.sonarr_client.get_series_by_title.assert_called_once_with('Breaking Bad')
    ...
    def test_test_specific_episode_episode_not_found(self, monitor):
        ...
        monitor.sonarr_client.get_series_by_title.return_value = {'id': 10, 'title': 'Test Series'}
...
class TestMonitoringLoop:
    ...
    def test_run_once_success(self, monitor):
        ...
        with patch.object(monitor, 'process_stuck_imports', return_value={'processed': 1, 'forced': 0, 'removed': 1}) as mock_process:
            with patch.object(monitor, 'check_repeated_grabs', return_value=0) as mock_check:
    ...
    def test_run_once_error(self, monitor):
        ...
        with patch.object(monitor, 'process_stuck_imports', side_effect=Exception("Processing error")):
            result = monitor.run_once()
            assert result is False
    ...
    def test_run_continuous_single_cycle(self, mock_sleep, monitor):
        ...
        with patch.object(monitor, 'process_stuck_imports', return_value={'processed': 0, 'forced': 0, 'removed': 0}):
    ...
    def test_run_continuous_with_webhook(self, mock_sleep, monitor):
        ...
        with patch.object(monitor, 'start_webhook_server', return_value=True) as mock_start:
            with patch.object(monitor, 'process_stuck_imports', return_value={'processed': 0, 'forced': 0, 'removed': 0}):
                # Stop after first cycle
                def stop_after_first(*args):
                    monitor.running = False
...
class TestCompleteWorkflow:
    ...
    def test_complete_processing_cycle(self, monitor, mock_sonarr_queue_item):
        ...
        with patch.object(monitor, 'sonarr_client') as mock_client:
            with patch.object(monitor, '_identify_stuck_items', return_value=stuck_items):
                with patch.object(monitor, 'analyzer') as mock_analyzer:
                    ...
                    mock_client.force_import.return_value = True

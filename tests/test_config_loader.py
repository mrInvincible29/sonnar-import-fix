...
from src.config.loader import ConfigLoader, ConfigurationError, DEFAULT_CONFIG
...
class TestConfigLoader:
        ...
        with patch('src.config.loader.load_dotenv'):
            with patch.dict('os.environ', {}, clear=True):
                with patch('src.config.loader.Path') as mock_path_class:
                    ...
                    mock_path_instance.exists.side_effect = lambda: True  # config.yaml exists
    ...
    def test_init_finds_config_in_config_dir(self):
        ...
        mock_config = {'sonarr': {'url': 'http://config-dir:8989', 'api_key': 'test-key'}}
            ...
            with patch.dict('os.environ', {}, clear=True):
                with patch('src.config.loader.Path') as mock_path_class:
                    def mock_path_exists(path_str):
                        ...
                        path_instance.exists.return_value = str(path_str) == 'config/config.yaml'
...
class TestConfigValidation:
    ...
    def test_validation_invalid_url_format(self):
        ...
        with patch.dict(os.environ, {
            'SONARR_URL': 'invalid-url',
            'SONARR_API_KEY': 'valid-key-123'
        }, clear=True):
            with patch('src.config.loader.load_dotenv'):
                with patch.object(Path, 'exists', return_value=False):
                    with pytest.raises(ConfigurationError) as exc_info:
...
class TestSensitiveValueMasking:
    ...
    def test_mask_api_key(self):
        ...
        with patch.dict('os.environ', test_env, clear=True):
            with patch('src.config.loader.load_dotenv'):
                with patch.object(Path, 'exists', return_value=False):
                    with patch('src.config.loader.secrets.token_urlsafe', return_value='mock-webhook-secret'):
                        config = ConfigLoader()
        ...
        with patch.dict('os.environ', test_env, clear=True):
            with patch('src.config.loader.load_dotenv'):
                with patch.object(Path, 'exists', return_value=False):
                    ...
                    assert masked['api']['endpoint'] == 'http://api.test.com'  # Not sensitive
        ...
        with patch.dict('os.environ', test_env, clear=True):
            with patch('src.config.loader.load_dotenv'):
                with patch.object(Path, 'exists', return_value=False):
                    ...
                    assert masked['config']['endpoints'] == ['http://test1.com', 'http://test2.com']
        ...
        with patch.dict('os.environ', test_env, clear=True):
            with patch('src.config.loader.load_dotenv'):
                with patch.object(Path, 'exists', return_value=False):
                    ...
                    assert config._convert_env_value('http://test.com') == 'http://test.com'
...
class TestCompleteConfigScenarios:
    ...
    def test_config_file_with_env_override(self, temp_config_file):
        ...
        with patch.dict(os.environ, {
            'SONARR_URL': 'http://env-override:9999',
            'WEBHOOK_PORT': '9090'
        ...
        with patch.dict(os.environ, {
            ...
            with patch('src.config.loader.load_dotenv'):
                with patch.object(Path, 'exists', return_value=False):
                    ...
                    assert config.get('sonarr.url') == 'http://localhost:8989'
                    assert config.get('sonarr.api_key') == 'valid-key-32-chars-long-123456'
...
class TestConfigIntegration:
    ...
    def test_development_config_with_file(self, temp_config_file):
        ...
        with patch.dict(os.environ, {
            'LOG_LEVEL': 'DEBUG'  # Only override log level

...
class TestCompleteWorkflow:
    ...
    def full_system(self, mock_config, test_env_vars):
        ...
        with patch('src.config.loader.load_dotenv'), \
             ...
             patch('src.config.loader.secrets.token_urlsafe', return_value='generated-secret'):
            config = ConfigLoader()
        
        # Create real components (but with mocked APIs)
        with requests_mock.Mocker() as m:
            # Mock all required API endpoints
            m.get('http://test-sonarr:8989/api/v3/system/status', 
                  json={'version': '3.0.0'})
    ...
    def test_monitor_initialization_and_configuration_test(self, full_system):
        ...
        with requests_mock.Mocker() as m:
            ...
            m.get('http://test-sonarr:8989/api/v3/system/status', 
                  json={'version': '3.0.0'})
            m.get('http://test-sonarr:8989/api/v3/customformat', 
                  json=[{'id': 1, 'name': 'HDR'}])
            m.get('http://test-sonarr:8989/api/v3/qualityprofile', 
                  json=[{'id': 1, 'name': 'HD-1080p'}])
            m.get('http://test-sonarr:8989/api/v3/series', 
    ...
    def test_complete_queue_processing_workflow(self, full_system):
        ...
        with requests_mock.Mocker() as m:
            # Mock queue API
            m.get('http://test-sonarr:8989/api/v3/queue', 
                  json={'records': [queue_item]})
            
            # Mock history API
            m.get('http://test-sonarr:8989/api/v3/history', 
                  json={'records': history})
            
            # Mock episode details (no current file)
            m.get('http://test-sonarr:8989/api/v3/episode/100', 
                  json={'hasFile': False})
            ...
            m.get('http://test-sonarr:8989/api/v3/qualityprofile',
                  json=[{'id': 1, 'name': 'HD-1080p', 'formatItems': [
                      {'format': 1, 'score': 100}, {'format': 2, 'score': 50}
                  ]}])
            ...
            # Mock force import
            m.get('http://test-sonarr:8989/api/v3/manualimport', 
                  json=[{'id': 1, 'path': '/test.mkv'}])
            m.put('http://test-sonarr:8989/api/v3/manualimport', 
                  json={'success': True})
...
class TestWebhookToMonitorIntegration:
    ...
    def integrated_system(self, mock_config, test_env_vars):
        ...
        with patch('src.core.monitor.signal.signal'), \
             patch('src.core.monitor.SonarrClient') as MockClient:
    ...
    def test_import_failed_triggers_immediate_check(self, integrated_system):
        ...
        with patch('threading.Timer') as mock_timer:
            ...
            mock_timer.assert_called_once_with(5, monitor.check_episode_queue, args=[100])
...
class TestErrorHandlingIntegration:
    ...
    def test_api_failure_recovery(self, error_prone_system):
        ...
        # First call fails
        with requests_mock.Mocker() as m:
            m.get('http://unreachable:8989/api/v3/queue', 
...
class TestPerformanceIntegration:
    ...
    def test_large_queue_processing(self, performance_system):
        ...
        with requests_mock.Mocker() as m:
            m.get('http://test-sonarr:8989/api/v3/queue', 
                  json={'records': large_queue})
            
            # Mock fast analysis (avoid actual API calls in analyzer)
            monitor.analyzer.analyze_queue_item = Mock(return_value=Decision(
                action='keep',
                grab_score=100,
                current_score=100,
                score_difference=0,
...
class TestMultiComponentScenarios:
    ...
    def test_webhook_grab_to_processing_pipeline(self, multi_component_system):
        ...
        monitor.sonarr_client.force_import = Mock(return_value=True)
    ...
    def test_concurrent_webhook_and_monitoring(self, multi_component_system):
        ...
        monitor.process_stuck_imports = Mock(return_value={'processed': 0, 'forced': 0, 'removed': 0})
...
class TestConfigurationIntegration:
    ...
    def test_environment_override_integration(self, test_env_vars):
        ...
        assert config.get('webhook.secret') == 'test-secret-123456789012345678901234567890'
        ...
        assert monitor.sonarr_client.api_key == 'test-key-123456789012345678901234567890'
        assert monitor.analyzer.force_import_threshold == 10
        
        # Test webhook server
        webhook_server = WebhookServer(monitor, config)
        assert webhook_server.webhook_secret == 'test-secret-123456789012345678901234567890'
